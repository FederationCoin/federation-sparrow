...
public class ServerSettingsController extends SettingsDetailController {
    ...
    public void initializeView(Config config) {
        ...
        coreDataDirSelect.setOnAction(event -> {
            ...
            directorChooser.setTitle("Select federationcoind Data Directory");
    ...
    private void showConnectionFailure(Throwable exception) {
        ...
        } else if(reason != null && (reason.contains("Check if federationcoind is running") || reason.contains("Could not connect to federationcoind RPC"))) {
            reason += "\n\nSee https://federationcoin.org";
        ...
        } else if(reason != null && (reason.startsWith("Cannot find federationcoind cookie file at"))) {
    ...
    private void showRemoteCoreServerWarning(Config config) {
        ...
        if(!coreServerWarningShown && config.getServerType() == ServerType.BITCOIN_CORE && isRemoteNode(coreServer)) {
            ...
            StringBuilder warning = new StringBuilder("federationcoind at " + coreServer.getHostAndPort() + location + " this computer or your local network.\n");
            if(AppServices.isUsingProxy()) {
                String proxy = config.isUseProxy() ? "configured proxy" : "internal Tor proxy";
                warning.append("\nConnections to federationcoind are made directly, and the " + proxy + " is only used for onion addresses. ");
            ...
            warning.append("\nConnecting to the federationcoind RPC interface over an untrusted network is not recommended by either Sparrow or federationcoind. ");
            ...
            AppServices.showWarningDialog("Remote federationcoind node", warning.toString());
        }
    }
    ...
    public void cormorantSyncStatus(CormorantSyncStatusEvent event) {
        ...
        if(connectionService != null && connectionService.isRunning() && event.getProgress() < 100) {
            ...
            testResults.appendText("\nThe connection to the federationcoind node was successful, but it is still syncing and cannot be used yet.");
