...
Instead, report them privately using GitHub's [Security tab](https://github.com/FederationCoin/federation-sparrow/security/advisories/new) for this repository. This creates a private security advisory visible only to you and the maintainers.

If you are unable to use GitHub, you may email the maintainers via the FederationCoin org. Sensitive reports can be encrypted to the key the releases are signed with, published on any [release](https://github.com/FederationCoin/federation-sparrow/releases).
...
- The Federation Sparrow version, operating system, and how it was installed (installer, package, or built from source)
...
Federation Sparrow follows the [Bitcoin Core Security Disclosure Policy](https://bitcoincore.org/en/security-advisories/). Reports are assigned one of four severity levels, which determines when details are made public:
...
There is currently no bug bounty program for Federation Sparrow.

## Supported Versions

Security fixes are made against the latest release only. Users are encouraged to run the most recent version, available from the [releases](https://github.com/FederationCoin/federation-sparrow/releases) page.
...
This policy covers Federation Sparrow and its submodules, [Drongo](https://github.com/FederationCoin/drongo) (protocol and wallet primitives) and [Lark](https://github.com/FederationCoin/lark) (USB hardware wallet interaction). Report issues in any of them here rather than on the submodule repositories, so that a fix and a release can be coordinated.
